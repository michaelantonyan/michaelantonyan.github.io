 ...
 * 
 * 222artist_portalA.php allows users with artist accounts to upload images and modify data 
	...
	$_SESSION['logged'] )  //Check the flag variable to see if user has been logged in
{
	...
	elseif (mysql_num_rows($result))
	{
		...
		if ($password == $row[1])
		{
			...
			echo "Click <a href='222logoutA.php'>here to Log Out</a>...<br />";			
			echo "$row[2] $row[3] : Hi $row[2],
				you are now logged in as '$row[0]'";
			// Creates form fields to change artist info using the POST method.
			echo '<form action="222artist_portalA.php" method="post" enctype="multipart/form-data"><pre>';
			echo 'First Name <input type="text" name="firstname" />	' . $row[2];
			echo '<br />Last Name <input type="text" name="lastname" />	' . $row[3];
			...
			if ($_FILES)
			{
				...
				if (!file_exists("/home/ads/" . $dbname . "/public_html/" . $username . "/"))
				{
					...
					mkdir("/home/ads/" . $dbname . "/public_html/" . $username . "/", 0777, true);
				}
				...
				if ($name != '')
				{
					...
					move_uploaded_file($_FILES['filename']['tmp_name'], "/home/ads/" . $dbname . "/public_html/" . $username . "/" . $name);
					...
					echo "Uploaded image '$username/$name'<br /><img src='http://$domain/~$dbname/$thumbname' />";
					...
					if (!mysql_num_rows($result))
					{
						...
						$query = "INSERT INTO images(username, filename, thumbname) VALUES('" . 
							$username . "', '" . $name . "', '" . $no_extension_name['filename'] . "thumb.jpg')";
						// Run the python script to create a thumbnail image of the uploaded image.
						popen('python thumbs.py ' . $username . ' ' . $name . ' ' . $dbname, 'w'); 
